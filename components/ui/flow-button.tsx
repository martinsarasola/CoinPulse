...
export function FlowButton({ text = "Modern Button" }: { text?: string }) {
  ...
  return (
    <button
      className={`group relative flex items-center gap-1 overflow-hidden rounded-[100px] border-[1.5px] ${
        isMobile ? "border-black dark:border-white" : "border-white"
      }  bg-transparent px-8 py-3 text-sm font-semibold text-[#111111] cursor-pointer transition-all duration-[600ms] ease-[cubic-bezier(0.23,1,0.32,1)] hover:border-transparent hover:text-white hover:rounded-[12px] active:scale-[0.95]`}
    >
      {/* Left arrow (arr-2) */}
      ...
      <span
        className={`relative z-[1] -translate-x-3 group-hover:translate-x-3 transition-all duration-[800ms] ease-out ${
          isMobile ? "text-black dark:text-white" : "text-white"
        }`}
      >
        {text}
      </span>
      ...
      <ArrowRight
        className={`absolute w-4 h-4 right-4 ${
          isMobile ? "stroke-black dark:stroke-white" : "stroke-white"
        }  fill-none z-[9] group-hover:right-[-25%] group-hover:stroke-white transition-all duration-[800ms] ease-[cubic-bezier(0.34,1.56,0.64,1)]`}
      />
    </button>
  );
}
